...
#                    plot.scatter (x,
#                                  main=NULL,
#                                  proj=c(1,2), boxes=FALSE, steps=x$cvfit$cv.nsteps,
...
plot.scatter.PRSP <- function(x,
  ...
  if (x$plot) {
    ...
    scatterplot <- function(object, main,
        ...
        eqscplot(x=X, type="p", main=NULL, xlab=X.names[1], ylab=X.names[2], ...)
        if (splom) {
            for (i in 1:L) {
                w <- object$cvfit$cv.boxind[steps[i],]
                points(x=object$x[w,proj], type="p", pch=pch, cex=cex, col=box.col, ...)
            }
        }
        if (boxes) {
