...
This branch (master) is the  default one, that hosts the current development release (version 0.7.6) of the survival bump hunting procedure 
that implements the case of a survival response. Note that `PRIMsrc` is still a non-production release and that version 0.7.6 implements 
...
PRIMsrc (>= 0.7.6) requires R-3.0.2 (2013-09-25). It was built and tested under R version 3.5.1 (2018-07-02) and Travis CI. 
...
* To install the stable version (0.7.6) of `PRIMsrc` from the [CRAN](https://CRAN.R-project.org/package=PRIMsrc) repository, 
simply download and install the current version (0.7.6) from the CRAN repository:

```{r}
install.packages("PRIMsrc")
```

* Alternatively, you can install the most up-to-date development version (>= 0.7.6) of `PRIMsrc` from the [GitHub](https://github.com/jedazard/PRIMsrc) repository, 
